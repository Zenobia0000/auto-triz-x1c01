...
當自然語言可能映射到多個候選參數時，使用以下加權評分進行消歧：

**Step M1：** 列出所有候選參數（匹配映射提示、說明欄、常見工程對應欄的任一項）

**Step M2：** 對每個候選參數 Pi 評分（每項 0-2 分）：

| 評分維度 | 權重 | 0 分 | 1 分 | 2 分 |
|:---------|:----:|:-----|:-----|:-----|
| 關鍵詞匹配 | 0.40 | 映射提示表中無對應 | 部分匹配（同義詞或上位概念） | 映射提示表中精確匹配 |
| 類別對齊 | 0.15 | 不同類別（如用能量類描述幾何問題） | 相鄰類別 | 同類別 |
| 說明欄重疊 | 0.25 | 無內容詞重疊 | 1-2 個詞重疊 | 3+ 個詞重疊或核心概念一致 |
| 工程對應重疊 | 0.20 | 常見工程對應欄無相關 | 弱關聯 | 常見工程對應欄直接涵蓋使用者描述的場景 |

**Step M3：** 計算加權分數 Score(Pi) = Σ(權重 × 分數/2)，歸一化至 [0, 1]

**Step M4：** 決策規則
- 最高分 ≥ 0.70 且與第二名差距 ≥ 0.15 → **自動選擇**
- 最高兩名差距 < 0.15 → **呈現兩者分數，詢問使用者確認**
- 無候選超過 0.40 → **要求使用者換方式描述或直接指定物理量**
- 若最終映射為兩組，則對兩組分別執行矩陣查表，取原理聯集

**Step M5：** 在 session 報告中記錄完整評分表

```markdown
#### 參數映射評分（{改善/惡化}）
| 候選 | P# | 關鍵詞(×0.40) | 類別(×0.15) | 說明(×0.25) | 工程對應(×0.20) | 總分 | 選中? |
|:-----|:---|:-------------|:-----------|:-----------|:---------------|:-----|:------|
| {名} | {#} | {0/1/2} | {0/1/2} | {0/1/2} | {0/1/2} | {分} | {是/否} |
```
...
#### 2.3.0 原理排序評分表（強制 — 候選 ≥ 2 個時必做）

矩陣返回多個候選原理時，使用三維加權評分排序：

```
PrincipleScore = 0.40 × PhysicsRelevance + 0.30 × DomainFit + 0.30 × Feasibility
```

**維度 1: 物理相關度 PhysicsRelevance (0-1.0)**

| 分數 | 條件 |
|:-----|:-----|
| 1.0 | 原理的控制方程直接治理 OZ 中的變數 |
| 0.5 | 原理操作在相關物理域（如：熱原理用於有熱耦合的電磁 OZ） |
| 0.0 | 無物理連結（如：重力原理用於純軟體時序問題） |

**維度 2: 領域適配度 DomainFit (0-1.0)**

| 領域 | 強適配原理 (1.0) | 中適配 (0.5) |
|:-----|:----------------|:-------------|
| 機械/結構 | 1,2,3,4,5,7,8,9,14,15,40 | 6,10,11,12,17 |
| 電子/電氣 | 18,19,20,28,31,32,37 | 1,2,3,13,35 |
| 熱/能量 | 15,19,22,34,36 | 10,14,17,18,35 |
| 化學/材料 | 31,35,36,38,39,40 | 3,5,15,17 |
| 軟體/控制 | 13,15,19,20,24,25,26 | 1,2,10,28,32 |

未列出的原理-領域組合 → 0.25（TRIZ 跨域遷移不設零分）

**維度 3: 可行性 Feasibility (0-1.0)**

三個二元問題，各值 0.33：
- 可寫出控制方程？（是=0.33，否=0）
- KB 中有匹配 OZ 物理尺度的跨域範例？（是=0.33，否=0）
- 可定義 ≤2 週的驗證實驗？（是=0.34，否=0）

**決策規則：**
- 降序排列所有候選原理
- 最高分為「主方向」；與最高分差距 ≤ 0.10 者為「共主方向」
- 低於 0.30 者淘汰並說明原因
- 攜帶前 2-3 名進入 Step 3
...
|:-----|:-----|:-----------|:----------|:------------|:-----|:-----|
| #{no} | {名} | {0-1} | {0-1} | {0-1} | {score} | {#} |
```

**品質檢查：** 候選方向中至少有一個能寫出控制方程。禁止純類比（「像 XX 一樣」）。

#### 2.3.1 外部數據驗證（Injection Point A — 強制）

候選方向確定後，對每個包含**具體數值聲明**的候選方向執行外部驗證：

```
對每個候選方向中的數值聲明（GFLOPs、材料性質、效率%、延遲 ms、物性值等）：

1. 使用 WebSearch 工具查詢：
   "{技術/材料名} {物性/指標} {聲明值} benchmark OR datasheet OR arxiv OR handbook"
   
2. 根據搜尋結果分類：
   - VERIFIED：找到可靠來源且數值吻合（偏差 ≤ 5%）
     → 記錄為 Evidence Registry 條目，Confidence = HIGH
   - APPROXIMATE：找到來源但數值偏差 5-20%
     → 記錄實際值和 LLM 估計值，Confidence = MEDIUM
   - UNVERIFIED：未找到來源或偏差 > 20%
     → 標記 Confidence = LOW，在候選方向描述中加註 [UNVERIFIED]

3. 使用 WebFetch 工具取得關鍵來源的完整資料（如 arxiv 摘要、datasheet PDF）

4. 產出格式 — 每個候選方向附帶驗證摘要：
   | 聲明 | LLM 估計值 | 外部驗證值 | 來源 | 狀態 |
   |:-----|:----------|:----------|:-----|:-----|
   | {描述} | {估計} | {實際} | {URL/DOI} | VERIFIED/APPROXIMATE/UNVERIFIED |
```
...
```
PxScore(Xi) = 0.50 × Sensitivity(Xi) + 0.25 × Controllability(Xi) + 0.25 × CausalDirectness(Xi)
```

**維度 1: 靈敏度 Sensitivity (0-1.0)**

| 子問題 | 0 | 0.5 | 1.0 |
|:-------|:--|:----|:----|
| Xi↑ → P1 方向的影響 | 無影響或同向於 P2 | 弱/間接影響 | 強/直接影響，反向於 P2 |
| Xi↑ → P2 方向的影響 | 無影響或同向於 P1 | 弱/間接影響 | 強/直接影響，反向於 P1 |

Sensitivity = (SubQ1 + SubQ2) / 2。**門檻：兩個子分數都必須 > 0**，否則 Xi 不是有效 Px。

**若有方程式可用：** P1 = f(Xi), P2 = g(Xi)
- Sensitivity = min(|∂f/∂Xi|_norm, |∂g/∂Xi|_norm)，正規化至候選中的最大值

**維度 2: 可控度 Controllability (0-1.0)**

| 分數 | 條件 |
|:-----|:-----|
| 1.0 | Xi 可由設計參數直接設定（幾何尺寸、材料選擇、電壓設定點） |
| 0.5 | Xi 可透過製程參數或反饋控制間接影響 |
| 0.0 | Xi 是湧現性質，無法獨立操控（如「系統熵」） |

**維度 3: 因果直接度 CausalDirectness (0-1.0)**

Xi 與 P1/P2 之間的中間變數數量：
- 0 個中間變數（Xi 直接出現在 P1 和 P2 的方程中）→ 1.0
- 1 個中間變數 → 0.7
- 2 個中間變數 → 0.4
- 3+ 個中間變數 → 0.1

**決策規則：**
- PxScore ≥ 0.60 且兩個靈敏度子分數 > 0 → 有效 Px 候選
- 選擇最高 PxScore 者為 Px
- 若無候選超過 0.60 → 標記「弱 Px」，建議使用者提供物理方程或進入下方三種 fallback

**產出格式：**

```markdown
#### Px 候選評分
| Xi | Sens-P1 | Sens-P2 | Sensitivity | Controllability | CausalDirect | PxScore | 選中? |
|:---|:--------|:--------|:------------|:----------------|:-------------|:--------|:------|
| {變數} | {0/0.5/1} | {0/0.5/1} | {avg} | {0/0.5/1} | {0.1-1.0} | {score} | {是/否} |
```
...
對四種分離策略各回答 3 個診斷問題（0/0.5/1.0），取平均為該策略分數：

**時間分離診斷：**

| 問題 | 0 | 0.5 | 1.0 |
|:-----|:--|:----|:----|
| T1: 系統是否有 A 和 ¬A 在不同時段被需要的情況？ | 無明確時相 | 時相存在但顯著重疊 | 明確不重疊的時相 |
| T2: OZ 中是否存在相關時間常數 τ？ | 無可辨識的 τ | τ 存在但接近操作頻率 | τ >> 或 << 操作頻率 |
| T3: 在所需頻率下 A↔¬A 切換是否物理可行？ | 無法切換 | 可切換但損耗大 | 可低損耗切換 |

**空間分離診斷：**

| 問題 | 0 | 0.5 | 1.0 |
|:-----|:--|:----|:----|
| S1: 需要 A 和 ¬A 的位置在 OZ 中是否空間分離？ | 同一點 | 部分重疊區域 | 明確不同區域 |
| S2: 空間梯度 (∇P ≠ 0) 是否可建立且維持？ | 無法建立梯度 | 梯度可建立但不穩定 | 穩定梯度可實現 |
| S3: A 區域與 ¬A 區域的邊界是否明確？ | 模糊/動態邊界 | 半明確邊界 | 銳利、可設計的邊界 |

**條件分離診斷：**

| 問題 | 0 | 0.5 | 1.0 |
|:-----|:--|:----|:----|
| C1: Px 是否對某可辨識控制參數 X 有非線性回應？ | 僅線性回應 | 弱非線性 | 強非線性或相變 |
| C2: 控制參數 X 是否可在運行中被刻意調變？ | X 固定 | X 自然變化但不可控 | X 可被刻意控制 |
| C3: A↔¬A 狀態轉換是否可逆？ | 不可逆 | 部分可逆（有遲滯） | 完全可逆 |

**整體局部分離診斷：**

| 問題 | 0 | 0.5 | 1.0 |
|:-----|:--|:----|:----|
| W1: 巨觀性質 A 能否從微觀 ¬A 元素湧現？ | 無已知湧現機制 | 理論機制存在 | 已有工程實例 |
| W2: 所需微結構是否可製造？ | 超出當前製造能力 | 需特殊製程 | 標準製程可達 |
| W3: 湧現性質是否有可預測的 scaling law？ | 無預測模型 | 有經驗關聯式 | 有解析模型 |

**選擇與嵌套規則：**

```
1. 計算四個分數，降序排列
2. 主策略 = 最高分（必須 ≥ 0.50 才可行）
3. 若無策略 ≥ 0.50 → 分離不可行，回到 Px 候選列表嘗試下一個
4. 若第二高分 ≥ 0.50 且兩策略正交 → 建議嵌套分離
   正交組合：時間+空間、時間+條件、空間+條件（永遠正交）
   整體局部可嵌套於任何其他策略
5. 最多 2 層嵌套。若 3+ 策略 ≥ 0.50 → 取最高兩個嵌套，第三個記為 fallback
```
...
| 主策略 | 次策略 | 典型場景 | 範例 |
|:-------|:-------|:---------|:-----|
| 條件 | 空間 | 不同條件路由到不同路徑，各路徑有空間梯度 | 級聯推論：信心度路由 → ROI 裁切 |
| 時間 | 條件 | 時間階段各有條件依賴行為 | 預熱階段(條件) → 穩態階段(時間) |
| 整體局部 | 空間 | 複合結構中子區域需不同局部策略 | FGM 各層不同晶粒結構 |
...
| 策略 | Q1 | Q2 | Q3 | 分數 | 可行? |
|:-----|:---|:---|:---|:-----|:------|
| 時間分離 | {0/0.5/1} | {0/0.5/1} | {0/0.5/1} | {avg} | {是/否} |
| 空間分離 | {0/0.5/1} | {0/0.5/1} | {0/0.5/1} | {avg} | {是/否} |
| 條件分離 | {0/0.5/1} | {0/0.5/1} | {0/0.5/1} | {avg} | {是/否} |
| 整體局部 | {0/0.5/1} | {0/0.5/1} | {0/0.5/1} | {avg} | {是/否} |
...
## 解法數據驗證（Injection Point B — 強制）

Step 3 完成解法方案後、寫入產出格式前，對解法中的所有數值聲明執行外部驗證：

### B.1 跨域實例驗證

對每個引用的跨域實例（來自 KB 或 LLM 推理），驗證其聲明的物理效應和數值：

```
1. WebSearch: "{效應名} {聲明物性} review OR handbook OR textbook"
2. 若跨域遷移涉及尺度變化（如：微觀→巨觀、實驗室→產線），
   明確陳述 scaling law 並驗證是否成立
3. 記錄至 Evidence Registry
```

### B.2 解法參數驗證

對解法方案中的 F（場）和 S（物質）參數值：

```
1. F 的控制方程中的常數/係數：
   WebSearch: "{場類型} {方程名} {係數} reference value"
2. S 的材料性質：
   WebSearch: "{材料名} {物性} datasheet OR materials database"
3. 系統級性能預測（延遲、效率、FNR 等）：
   WebSearch: "{系統架構} {指標} benchmark published results"
```

### B.3 驗證摘要

所有 Injection B 驗證結果匯入 Evidence Registry。統計：

```
- VERIFIED 聲明數：{N} / {Total}
- APPROXIMATE 聲明數：{N} / {Total}
- UNVERIFIED 聲明數：{N} / {Total}
- 驗證覆蓋率：{(VERIFIED + APPROXIMATE) / Total × 100}%
```

**門檻：** 驗證覆蓋率 < 40% 時，在產出中加註警語：「⚠ 本方案數據驗證覆蓋率偏低，建議在 Step 4 前補充文獻調研或原型實驗。」
...
**Evidence Registry 規則：**
- **HIGH**：同儕審查論文、官方 datasheet、NIST/標準手冊
- **MEDIUM**：廠商技術文章、研討會論文、Wikipedia（有引用來源）
- **LOW**：LLM 參數記憶，未經外部確認
- 每個數值聲明（GFLOPs、材料性質、效率%、延遲 ms 等）**必須**標註 Claim ID，如：`~5 GFLOPs [C-001]`
- 禁止在解法方案中出現無 Claim ID 標註的數值聲明
